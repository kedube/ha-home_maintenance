...
`;var d1=C=>{let[H]=C.split("T"),[V,L,e]=H.split("-").map(Number);return new Date(V,L-1,e)},p1=C=>{let H=C.trigger_type??"time";return H==="time"||H==="date"},R1=C=>`${C.progress_current??0} / ${C.progress_target??0}`,U2=(C,H,V)=>{let L=C===1?H.slice(0,-1):H;return`${C} ${i(`intervals.${L}`,V)}`},G5=(C,H)=>{let V=C.trigger_type??"time";return V==="count"?i("intervals.every_uses",H,"{value}",String(C.count_threshold??0)):V==="runtime"?i("intervals.every_runtime",H,"{value}",String(C.runtime_threshold??0)):U2(C.interval_value,C.interval_type,H)};var U5=(C,H,V,L)=>{let e=V.trim().toLowerCase();if(!e&&!L.length)return C;let r=new Map;return L.length&&H.forEach(M=>r.set(M.unique_id,M.labels)),C.filter(M=>{if(e&&!`${M.title}
${M.description??""}
${M.group_id??""}`.toLowerCase().includes(e))return!1;if(L.length){let t=r.get(M.id)??[];if(!L.some(a=>t.includes(a)))return!1}return!0})},Y1=class{constructor(H,V){this._fn=H;this._ms=V}schedule(){this.cancel(),this._timer=setTimeout(()=>{this._timer=void 0,this._fn()},this._ms)}cancel(){this._timer!==void 0&&clearTimeout(this._timer),this._timer=void 0}};var C2=["days","weeks","months","years"],W5={days:1,weeks:7,months:30,years:365},$5=500;function Q5(C){return{days:i("intervals.days",C),weeks:i("intervals.weeks",C),months:i("intervals.months",C),years:i("intervals.years",C)}}var k7=["title","description","interval_value","interval_type","last_performed","icon","group_id"],j5=C=>{let H=[],V=[],L="",e=!1,r=0,M=()=>{V.push(L),L=""},t=()=>{M(),V.some(a=>a.trim()!=="")&&H.push(V),V=[]};for(;r<C.length;){let a=C[r];if(e){if(a==='"'){if(C[r+1]==='"'){L+='"',r+=2;continue}e=!1,r+=1;continue}L+=a,r+=1;continue}if(a==='"'&&L===""){e=!0,r+=1;continue}if(a===","){M(),r+=1;continue}if(a===`
`||a==="\r"){a==="\r"&&C[r+1]===`
`&&(r+=1),t(),r+=1;continue}L+=a,r+=1}return(L!==""||V.length)&&t(),H},w7=/^\d{4}-\d{2}-\d{2}$/,q5=C=>{if(!C.length)return{tasks:[],errors:["The file is empty."]};let H=C[0].map(e=>e.trim().toLowerCase());if(!H.includes("title"))return{tasks:[],errors:['The header row must include a "title" column.']};let V=[],L=[];return C.slice(1).forEach((e,r)=>{let M=r+2,t=b=>{let w=H.indexOf(b);return w>=0?(e[w]??"").trim():""},a=t("title");if(!a){L.push(`Line ${M}: missing title.`);return}let A=t("interval_value"),n=A===""?30:Number(A);if(!Number.isFinite(n)||n<1){L.push(`Line ${M}: invalid interval_value "${A}".`);return}let l=t("interval_type").toLowerCase(),m=l===""?"days":l;if(!C2.includes(m)){L.push(`Line ${M}: invalid interval_type "${l}".`);return}let Z=t("last_performed");if(Z&&!w7.test(Z)){L.push(`Line ${M}: last_performed must be YYYY-MM-DD.`);return}V.push({title:a,description:t("description")||void 0,interval_value:Math.floor(n),interval_type:m,last_performed:Z||void 0,icon:t("icon")||void 0,group_id:t("group_id")||void 0})}),{tasks:V,errors:L}},T7=C=>{let H=/^[=+\-@\t]/.test(C)?`'${C}`:C;return/[",\n\r]/.test(H)?`"${H.replace(/"/g,'""')}"`:H},K5=C=>{let H=[k7.join(",")];return C.forEach(V=>{H.push([V.title,V.description??"",String(V.interval_value),V.interval_type,V.last_performed?V.last_performed.split("T")[0]:"",V.icon??"",V.group_id??""].map(T7).join(","))}),H.join(`\r
`)+`\r
`};var X5=C=>C.callWS({type:"config/entity_registry/list"}),J5=C=>C.callWS({type:"config/label_registry/list"}),Y5=C=>C.callWS({type:"home_maintenance/get_tasks"}),C3=(C,H)=>C.callWS({type:"home_maintenance/get_task",task_id:H}),H2=(C,H)=>C.callWS({type:"home_maintenance/add_task",...H}),H3=(C,H)=>C.callWS({type:"home_maintenance/remove_task",task_id:H}),V3=(C,H,V)=>C.callWS({type:"home_maintenance/complete_task",task_id:H,...V?{note:V}:{}}),V2=(C,H)=>C.callWS({type:"home_maintenance/update_task",...H}),L3=C=>C.callWS({type:"home_maintenance/get_groups"}),e3=(C,H)=>C.callWS({type:"home_maintenance/create_group",group_id:H}),r3=(C,H,V)=>C.callWS({type:"home_maintenance/rename_group",old_group_id:H,new_group_id:V}),M3=(C,H)=>C.callWS({type:"home_maintenance/delete_group",group_id:H}),t3=C=>C.callWS({type:"home_maintenance/get_config"}),i3=(C,H)=>C.connection.subscribeMessage(H,{type:"home_maintenance/subscribe_updates"});var B7=(C,H)=>{let V=C.language,L=p1(H),e=L?G5(H,V):R1(H);return{heading:i("panel.dialog.confirm_complete.title",V),message:i(L?"panel.dialog.confirm_complete.message":"panel.dialog.confirm_complete.message_progress",V,"{title}",H.title,"{interval}",e),confirmLabel:i("panel.dialog.confirm_complete.actions.confirm",V),cancelLabel:i("common.cancel",V),input:{label:i("panel.dialog.confirm_complete.note_label",V)},onConfirm:()=>{}}},a3=(C,H,V,L,e=r=>r())=>{let r=V.language;H?.open({...B7(V,L),onConfirm:M=>e(async()=>{try{await V3(V,L.id,M),f(C,i("panel.cards.current.alerts.complete_success",r,"{title}",L.title))}catch(t){console.error("Failed to complete task:",t),f(C,i("panel.cards.current.alerts.complete_error",r))}})})},o3=(C,H,V,L,e)=>{let r=V.language;H?.open({heading:i("panel.dialog.confirm_remove.title",r),message:i("panel.dialog.confirm_remove.message",r,"{title}",L?.title??""),confirmLabel:i("panel.dialog.confirm_remove.actions.confirm",r),cancelLabel:i("common.cancel",r),destructive:!0,onConfirm:async()=>{try{await H3(V,e)}catch(M){console.error("Failed to remove task:",M),f(C,i("panel.cards.current.alerts.remove_error",r))}}})};var A3="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z";var n3="M12 2C6.5 2 2 6.5 2 12S6.5 22 12 22 22 17.5 22 12 17.5 2 12 2M12 20C7.59 20 4 16.41 4 12S7.59 4 12 4 20 7.59 20 12 16.41 20 12 20M16.59 7.58L10 14.17L7.41 11.59L6 13L10 17L18 9L16.59 7.58Z";var d3="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z";var p3="M19,4H15.5L14.5,3H9.5L8.5,4H5V6H19M6,19A2,2 0 0,0 8,21H16A2,2 0 0,0 18,19V7H6V19Z";var l3="M20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18,2.9 17.35,2.9 16.96,3.29L15.12,5.12L18.87,8.87M3,17.25V21H6.75L17.81,9.93L14.06,6.18L3,17.25Z";var F1=class extends g{constructor(){super();this.items=[]}render(){return d`
...
`;var v3=(C,H,V,L)=>{try{return i(`${C.keyPrefix}.${H}.${V}`,C.hass.language)??L}catch{return L}},e2=(C,H)=>d`
    <div class="field ${H.name}">
        <div class="field-label">
            ${v3(C,H.name,"heading",H.name)}${H.required?" *":""}
        </div>
        <ha-selector
            .hass=${C.hass}
            .selector=${H.selector}
            .value=${C.data[H.name]}
            .helper=${v3(C,H.name,"helper","")}
...
`;var l1=()=>({title:"",trigger_type:"time",interval_value:"",interval_type:"days",last_performed:"",anchor_date:"",active_months:[],icon:"",label:[],tag:"",count_entity_id:"",count_threshold:"",runtime_entity_id:"",runtime_threshold:"",area:"",description:"",group_id:"",notifications_enabled:!1,notification_target:"",notification_time:"09:00",notification_url:"",notify_when:"due_and_overdue",notify_days_before_due:""}),u3=(C,H,V)=>({title:C.title,trigger_type:C.trigger_type??"time",interval_value:C.interval_value,interval_type:C.interval_type,last_performed:C.last_performed??"",anchor_date:C.anchor_date??"",active_months:(C.active_months??[]).map(String),icon:C.icon??"",label:V.map(L=>L.label_id),tag:C.tag_id??"",count_entity_id:C.count_entity_id??"",count_threshold:C.count_threshold??"",runtime_entity_id:C.runtime_entity_id??"",runtime_threshold:C.runtime_threshold??"",area:H?.area_id??"",description:C.description??"",group_id:C.group_id??"",notifications_enabled:C.notifications_enabled??!1,notification_target:C.notification_target??"",notification_time:C.notification_time??"09:00",notification_url:C.notification_url??"",notify_when:C.notify_when??"due_and_overdue",notify_days_before_due:C.notify_days_before_due??""}),E7=C=>({name:"trigger_type",required:!0,selector:{select:{options:[{value:"time",label:i("trigger_types.time",C)},{value:"date",label:i("trigger_types.date",C)},{value:"count",label:i("trigger_types.count",C)},{value:"runtime",label:i("trigger_types.runtime",C)}],mode:"dropdown"}}}),R7=C=>{let H;try{H=new Intl.DateTimeFormat(C,{month:"long"})}catch{H=new Intl.DateTimeFormat("en",{month:"long"})}return Array.from({length:12},(V,L)=>({value:String(L+1),label:H.format(new Date(2026,L,1,12))}))},F7=C=>({name:"active_months",selector:{select:{options:R7(C),multiple:!0,mode:"dropdown"}}}),x3=C=>({name:"interval_type",required:!0,selector:{select:{options:C2.map(H=>({value:H,label:Q5(C)[H]})),mode:"dropdown"}}}),D7=(C,H)=>C.trigger_type==="date"?[{name:"anchor_date",required:!0,selector:{date:{}}},{name:"interval_value",required:!0,selector:{number:{min:1,mode:"box"}}},x3(H)]:C.trigger_type==="count"?[{name:"count_entity_id",required:!0,selector:{entity:{}}},{name:"count_threshold",required:!0,selector:{number:{min:1,mode:"box"}}}]:C.trigger_type==="runtime"?[{name:"runtime_entity_id",required:!0,selector:{entity:{filter:{domain:"sensor"}}}},{name:"runtime_threshold",required:!0,selector:{number:{min:.1,step:.1,mode:"box"}}}]:[{name:"interval_value",required:!0,selector:{number:{min:1,mode:"box"}}},x3(H),F7(H)],W2=(C,H)=>({name:"group_id",selector:{select:{options:[{value:"",label:i("common.ungrouped",H)},...C.map(V=>({value:V,label:V}))],mode:"dropdown",custom_value:!0}}}),M2=(C,H)=>[{name:"title",required:!0,selector:{text:{}}},E7(H),...D7(C,H)],t2={name:"last_performed",selector:{date:{}}},i2=(C,H)=>[W2(C,H),{name:"icon",selector:{icon:{}}},{name:"tag",selector:{entity:{filter:{domain:"tag"}}}},{name:"area",selector:{area:{}}},{name:"label",selector:{label:{multiple:!0}}}],a2=C=>({name:"description",selector:{text:C?{multiline:!0}:{}}}),o2=(C,H,V)=>{let L={name:"notifications_enabled",selector:{boolean:{}}};return C.notifications_enabled?[L,{name:"notification_target",selector:{select:{options:[{value:"",label:i("common.none",V)},...H.map(e=>({value:e,label:e}))],mode:"dropdown",custom_value:!0}}},{name:"notify_when",selector:{select:{options:[{value:"due",label:i("notifications.when.due",V)},{value:"overdue",label:i("notifications.when.overdue",V)},{value:"due_and_overdue",label:i("notifications.when.due_and_overdue",V)}],mode:"dropdown"}}},...C.trigger_type==="time"||C.trigger_type==="date"?[{name:"notify_days_before_due",selector:{number:{min:1,mode:"box"}}}]:[],{name:"notification_time",selector:{time:{no_second:!0}}},{name:"notification_url",selector:{text:{}}}]:[L]},A2=C=>C.title?.trim()?C.trigger_type==="count"?!!(C.count_entity_id?.trim()&&C.count_threshold):C.trigger_type==="runtime"?!!(C.runtime_entity_id?.trim()&&C.runtime_threshold):C.trigger_type==="date"?!!(C.anchor_date?.trim()&&C.interval_value&&C.interval_type):!!(C.interval_value&&C.interval_type):!1,m1=C=>{if(!C){let a=new Date;return a.setHours(0,0,0,0),a.toISOString()}let[H,V,L]=C.split("T")[0].split("-"),e=Number(H),r=Number(V),M=Number(L);if(isNaN(e)||isNaN(r)||isNaN(M))return null;let t=new Date(e,r-1,M);return t.setHours(0,0,0,0),t.toISOString()},c3=C=>({notifications_enabled:C.notifications_enabled??!1,notification_target:C.notification_target?.trim()||null,notification_time:C.notification_time?.trim()||"09:00",notification_url:C.notification_url?.trim()||null,notify_when:C.notify_when||"due_and_overdue",notify_days_before_due:C.notify_days_before_due===""||C.notify_days_before_due==null?null:Number(C.notify_days_before_due)}),Z3=C=>{let H=C.trigger_type==="count",V=C.trigger_type==="runtime",L=C.trigger_type==="date",e=!H&&!V&&!L;return{trigger_type:C.trigger_type||"time",interval_value:H||V?1:Number(C.interval_value),interval_type:H||V?"days":C.interval_type,anchor_date:L&&C.anchor_date?.trim().split("T")[0]||null,active_months:e?(C.active_months??[]).map(Number):[],count_entity_id:H&&C.count_entity_id?.trim()||null,count_threshold:H?Number(C.count_threshold):0,runtime_entity_id:V&&C.runtime_entity_id?.trim()||null,runtime_threshold:V?Number(C.runtime_threshold):0}},h3=(C,H)=>{let V=Z3(C),L=C.trigger_type==="date"&&!C.last_performed?.trim();return{title:C.title.trim(),interval_value:V.interval_value,interval_type:V.interval_type,trigger_type:V.trigger_type,...L?{}:{last_performed:H},tag_id:C.tag?.trim()||void 0,icon:C.icon?.trim()||"mdi:calendar-check",labels:C.label??[],area_id:C.area?.trim()||void 0,description:C.description||void 0,group_id:C.group_id?.trim()||void 0,...V.anchor_date?{anchor_date:V.anchor_date}:{},...V.active_months.length?{active_months:V.active_months}:{},...V.count_entity_id?{count_entity_id:V.count_entity_id,count_threshold:V.count_threshold}:{},...V.runtime_entity_id?{runtime_entity_id:V.runtime_entity_id,runtime_threshold:V.runtime_threshold}:{},...c3(C)}},S3=(C,H)=>({title:C.title.trim(),...Z3(C),last_performed:H,icon:C.icon?.trim()||"mdi:calendar-check",labels:C.label,tag_id:C.tag?.trim()||null,area_id:C.area?.trim()||null,description:C.description??"",group_id:C.group_id?.trim()||null,...c3(C)});var a1=class extends g{constructor(){super(...arguments);this.groups=[];this._formData=l1();this._advancedOpen=!1;this._handleFieldChanged=(V,L)=>{L.stopPropagation(),this._formData={...this._formData,[V]:L.detail.value}};this._renderField=V=>e2({hass:this.hass,keyPrefix:"panel.cards.new.fields",data:this._formData,onChange:this._handleFieldChanged},V)}prefill(V){this._formData={...this._formData,...V}}async _handleAddTaskClick(){if(!A2(this._formData)){f(this,i("panel.cards.new.alerts.required",this.hass.language));return}let V=m1(this._formData.last_performed);if(V===null){f(this,i("common.invalid_date",this.hass.language));return}try{let L=this._formData.title.trim();await H2(this.hass,h3(this._formData,V)),this._formData=l1(),this.dispatchEvent(new CustomEvent("task-added",{detail:{title:L},bubbles:!0,composed:!0}))}catch(L){console.error("Failed to add task:",L),f(this,i("panel.cards.new.alerts.error",this.hass.language))}}render(){return this.hass?d`
            <div class="basic-row">
            ...
            >
                <div class="fields-grid">
                    ${this._renderField(t2)}
                    ${i2(this.groups,this.hass.language).map(this._renderField)}
                    ${this._renderField(a2(!1))}
                </div>
                ...
                <div class="fields-grid">
                    ${o2(this._formData,L2(this.hass),this.hass.language).map(this._renderField)}
                </div>
            </ha-expansion-panel>
        `:d``}};a1.styles=[k,r2,O`
    ...
    `],p([c()],a1.prototype,"hass",2),p([c({attribute:!1})],a1.prototype,"groups",2),p([h()],a1.prototype,"_formData",2);customElements.get("hm-task-form")||customElements.define("hm-task-form",a1);var P=class extends g{constructor(){super(...arguments);this.registry=[];this.labelRegistry=[];this.groups=[];this._taskId=null;this._formData=l1();this._history=[];this._handleFieldChanged=(V,L)=>{L.stopPropagation(),this._formData={...this._formData,[V]:L.detail.value}};this._renderField=V=>e2({hass:this.hass,keyPrefix:"panel.dialog.edit_task.fields",data:this._formData,onChange:this._handleFieldChanged},V)}async open(V){try{let L=await C3(this.hass,V),e=this.registry.find(M=>M.unique_id===L.id),r=e?this.labelRegistry.filter(M=>e.labels.includes(M.label_id)):[];this._formData=u3(L,e,r),this._history=L.history??[],this._taskId=L.id}catch(L){console.error("Failed to fetch task for edit:",L)}}async _handleSaveClick(){if(!this._taskId)return;if(!A2(this._formData)){f(this,i("panel.cards.new.alerts.required",this.hass.language));return}let V=m1(this._formData.last_performed);if(V===null){f(this,i("common.invalid_date",this.hass.language));return}try{await V2(this.hass,{task_id:this._taskId,updates:S3(this._formData,V)}),this._close()}catch(L){console.error("Failed to update task:",L),f(this,i("panel.dialog.edit_task.alerts.error",this.hass.language))}}_close(){this._taskId=null,this._formData=l1(),this._history=[]}async _handleTestNotification(){let V=this.registry.find(L=>L.unique_id===this._taskId);if(V)try{await this.hass.callService("home_maintenance","send_task_notification",{entity_id:V.entity_id})}catch(L){console.error("Failed to send test notification:",L),f(this,i("panel.dialog.edit_task.alerts.test_error",this.hass.language))}}render(){if(!this.hass||!this._taskId)return d``;let V=this.hass.language;return d`
            <ha-dialog
                open
            ...
            >
                ...
                <div class="fields-grid">
                    ${i2(this.groups,V).map(this._renderField)}
                    ${this._renderField(a2(!0))}
                </div>
                ...
                <div class="fields-grid">
                    ${o2(this._formData,L2(this.hass),V).map(this._renderField)}
                </div>
                ...
                ${this._history.length?d`
                    ...
                    <div class="history-scroll">
                        ${m3(this._history,this._history.length,this.hass.locale)}
                    </div>
                `:v}
    ...
    `],p([h()],s1.prototype,"_opts",2),p([T(".confirm-input")],s1.prototype,"_input",2);customElements.get("hm-confirm-dialog")||customElements.define("hm-confirm-dialog",s1);var F=class extends g{constructor(){super(...arguments);this.groups=[];this._newGroupName="";this._renamingGroup=null;this._renameValue=""}async _handleCreate(){let V=this._newGroupName.trim();if(V){if(this.groups.includes(V)){f(this,i("panel.cards.groups.alerts.exists",this.hass.language,"{title}",V));return}try{await e3(this.hass,V),this._newGroupName=""}catch(L){console.error("Failed to create group:",L),f(this,i("panel.cards.groups.alerts.error",this.hass.language))}}}_startRename(V){this._renamingGroup=V,this._renameValue=V}async _handleRename(){let V=this._renamingGroup,L=this._renameValue.trim();if(!V||!L||V===L){this._renamingGroup=null;return}if(this.groups.includes(L)){f(this,i("panel.cards.groups.alerts.exists",this.hass.language,"{title}",L));return}this._renamingGroup=null;try{await r3(this.hass,V,L)}catch(e){console.error("Failed to rename group:",e),f(this,i("panel.cards.groups.alerts.rename_error",this.hass.language))}}_handleDelete(V){let L=this.hass.language;this._confirmDialog?.open({heading:i("panel.cards.groups.confirm_delete_title",L),message:i("panel.cards.groups.confirm_delete",L,"{title}",V),confirmLabel:i("panel.cards.groups.actions.delete",L),cancelLabel:i("common.cancel",L),destructive:!0,onConfirm:()=>this._deleteGroup(V)})}async _deleteGroup(V){try{await M3(this.hass,V)}catch(L){console.error("Failed to delete group:",L),f(this,i("panel.cards.groups.alerts.delete_error",this.hass.language))}}render(){if(!this.hass)return d``;let V=this.hass.language;return d`
...
${L.description}`.toLowerCase().includes(V)):$2}_selectTemplate(V){this.dispatchEvent(new CustomEvent("template-selected",{detail:{template:V},bubbles:!0,composed:!0})),this._close()}async _handleFilePicked(V){let L=V.target.files?.[0];if(!L)return;let e=await L.text(),r=q5(j5(e));this._csvRows=r.tasks,this._csvErrors=r.errors}async _handleImport(){if(!this._csvRows?.length||this._importing)return;this._importing=!0;let V=0,L=[];for(let e of this._csvRows)try{await H2(this.hass,{title:e.title,interval_value:e.interval_value,interval_type:e.interval_type,trigger_type:"time",last_performed:m1(e.last_performed??""),icon:e.icon||"mdi:calendar-check",...e.description?{description:e.description}:{},...e.group_id?{group_id:e.group_id}:{}}),V+=1}catch(r){console.error("Failed to import task:",e.title,r),L.push(e.title)}this.dispatchEvent(new CustomEvent("csv-imported",{detail:{created:V,failures:L},bubbles:!0,composed:!0})),this._close()}_renderCsvSection(){let V=this.hass.language;return d`
        ...
        `}render(){if(!this.hass||!this._open)return d``;let V=this.hass.language,L=this._filteredTemplates;return d`
            ...
            >
                ...
                <div class="template-list">
                    ${g3.map(e=>{let r=L.filter(M=>M.category===e);return r.length?d`
    ...
    `],p([c()],E.prototype,"hass",2),p([h()],E.prototype,"_open",2),p([h()],E.prototype,"_query",2),p([h()],E.prototype,"_csvRows",2),p([h()],E.prototype,"_csvErrors",2),p([h()],E.prototype,"_importing",2),p([T('input[type="file"]')],E.prototype,"_fileInput",2);customElements.get("hm-template-dialog")||customElements.define("hm-template-dialog",E);var N7=300,y=class extends g{constructor(){super(...arguments);this._loaded=!1;this.tasks=[];this.groups=[];this.config=null;this.registry=[];this.labelRegistry=[];this._search="";this._selectedLabels=[];this._reload=new Y1(()=>this._loadData(),N7)}connectedCallback(){super.connectedCallback(),this._initialize()}disconnectedCallback(){super.disconnectedCallback(),this._reload.cancel(),this._unsubscribe?.(),this._unsubscribe=void 0}async _initialize(){await D5(),this.config=await t3(this.hass),await this._loadData(),this._loaded=!0;try{this._unsubscribe=await i3(this.hass,()=>this._reload.schedule())}catch(V){console.error("Failed to subscribe to task updates:",V)}}async _loadData(){let[V,L,e,r]=await Promise.all([Y5(this.hass),L3(this.hass),X5(this.hass),J5(this.hass)]);this.tasks=V,this.groups=L,this.registry=e,this.labelRegistry=r}_handleComplete(V){let L=this.tasks.find(e=>e.id===V.detail.taskId);L&&a3(this,this._confirmDialog,this.hass,L)}_handleMenuAction(V){let{taskId:L,action:e}=V.detail;if(e==="edit")this._editDialog?.open(L);else if(e==="move"){let r=this.tasks.find(M=>M.id===L);r&&this._moveDialog?.open(r)}else e==="delete"&&this._handleRemove(L)}_handleRemove(V){let L=this.tasks.find(e=>e.id===V);o3(this,this._confirmDialog,this.hass,L,V)}_handleTaskAdded(V){f(this,i("card.add_task.added",this.hass.language,"{title}",V.detail?.title??""))}_handleTemplateSelected(V){let L=V.detail.template;this._taskForm?.prefill({title:L.title,description:L.description,trigger_type:"time",interval_value:L.interval_value,interval_type:L.interval_type,icon:L.icon}),this._taskForm?.scrollIntoView({behavior:"smooth",block:"center"})}_handleCsvImported(V){let{created:L,failures:e}=V.detail;f(this,i("panel.dialog.templates.imported",this.hass.language,"{count}",L)),e.length&&f(this,i("panel.dialog.templates.import_failed",this.hass.language,"{titles}",e.join(", ")))}_handleExportCsv(){let V=new Blob([K5(this.tasks)],{type:"text/csv"}),L=URL.createObjectURL(V),e=document.createElement("a");e.href=L,e.download="home_maintenance_tasks.csv",e.click(),URL.revokeObjectURL(L)}get _visibleTasks(){let V=this._visibleCache;if(V&&V.tasks===this.tasks&&V.registry===this.registry&&V.search===this._search&&V.labels===this._selectedLabels)return V.result;let L=U5(this.tasks,this.registry,this._search,this._selectedLabels);return this._visibleCache={tasks:this.tasks,registry:this.registry,search:this._search,labels:this._selectedLabels,result:L},L}get _labelsInUse(){let V=this._labelsInUseCache;if(V&&V.tasks===this.tasks&&V.registry===this.registry&&V.labelRegistry===this.labelRegistry)return V.result;let L=new Set,e=new Set(this.tasks.map(M=>M.id));this.registry.forEach(M=>{e.has(M.unique_id)&&M.labels.forEach(t=>L.add(t))});let r=this.labelRegistry.filter(M=>L.has(M.label_id));return this._labelsInUseCache={tasks:this.tasks,registry:this.registry,labelRegistry:this.labelRegistry,result:r},r}_toggleLabel(V){this._selectedLabels=this._selectedLabels.includes(V)?this._selectedLabels.filter(L=>L!==V):[...this._selectedLabels,V]}render(){return this.hass?this._loaded?d`
            <div class="header">
                <div class="toolbar">
