...
`;var N1=C=>{let[H]=C.split("T"),[V,L,M]=H.split("-").map(Number);return new Date(V,L-1,M)},c1=C=>`${C.progress_current??0} / ${C.progress_target??0}`,f2=(C,H,V)=>{let L=C===1?H.slice(0,-1):H;return`${C} ${i(`intervals.${L}`,V)}`},h5=(C,H)=>{let V=C.trigger_type??"time";return V==="count"?i("intervals.every_uses",H,"{value}",String(C.count_threshold??0)):V==="runtime"?i("intervals.every_runtime",H,"{value}",String(C.runtime_threshold??0)):f2(C.interval_value,C.interval_type,H)},I1=C=>Object.keys(C.services?.notify??{}).filter(H=>H!=="notify").map(H=>`notify.${H}`).sort((H,V)=>H.localeCompare(V)),V1=C=>customElements.get("ha-dialog-footer")?d`<ha-dialog-footer slot="footer">${C}</ha-dialog-footer>`:C,D1=class{constructor(H,V){this._fn=H;this._ms=V}schedule(){this.cancel(),this._timer=setTimeout(()=>{this._timer=void 0,this._fn()},this._ms)}cancel(){this._timer!==void 0&&clearTimeout(this._timer),this._timer=void 0}};var f5=C=>C.connection.sendMessagePromise({type:"tag/list"}),g5=C=>C.callWS({type:"config/entity_registry/list"}),O5=C=>C.callWS({type:"config/label_registry/list"}),y5=C=>C.callWS({type:"home_maintenance/get_tasks"}),b5=(C,H)=>C.callWS({type:"home_maintenance/get_task",task_id:H}),k5=(C,H)=>C.callWS({type:"home_maintenance/add_task",...H}),w5=(C,H)=>C.callWS({type:"home_maintenance/remove_task",task_id:H}),B5=(C,H)=>C.callWS({type:"home_maintenance/complete_task",task_id:H}),W1=(C,H)=>C.callWS({type:"home_maintenance/update_task",...H}),T5=C=>C.callWS({type:"home_maintenance/get_groups"}),P5=(C,H)=>C.callWS({type:"home_maintenance/create_group",group_id:H}),_5=(C,H,V)=>C.callWS({type:"home_maintenance/rename_group",old_group_id:H,new_group_id:V}),F5=(C,H)=>C.callWS({type:"home_maintenance/delete_group",group_id:H}),E5=C=>C.callWS({type:"home_maintenance/get_config"}),R5=(C,H)=>C.connection.subscribeMessage(H,{type:"home_maintenance/subscribe_updates"});var i0=(C,H)=>{let V=C.language,L=(H.trigger_type??"time")==="time",M=L?h5(H,V):c1(H);return{heading:i("panel.dialog.confirm_complete.title",V),message:i(L?"panel.dialog.confirm_complete.message":"panel.dialog.confirm_complete.message_progress",V,"{title}",H.title,"{interval}",M),confirmLabel:i("panel.dialog.confirm_complete.actions.confirm",V),cancelLabel:i("common.cancel",V),onConfirm:()=>{}}},D5=(C,H,V,L,M=e=>e())=>{let e=V.language;H?.open({...i0(V,L),onConfirm:()=>M(async()=>{try{await B5(V,L.id),f(C,i("panel.cards.current.alerts.complete_success",e,"{title}",L.title))}catch(r){console.error("Failed to complete task:",r),f(C,i("panel.cards.current.alerts.complete_error",e))}})})},N5=(C,H,V,L,M)=>{let e=V.language;H?.open({heading:i("panel.dialog.confirm_remove.title",e),message:i("panel.dialog.confirm_remove.message",e,"{title}",L?.title??""),confirmLabel:i("panel.dialog.confirm_remove.actions.confirm",e),cancelLabel:i("common.cancel",e),destructive:!0,onConfirm:async()=>{try{await w5(V,M)}catch(r){console.error("Failed to remove task:",r),f(C,i("panel.cards.current.alerts.remove_error",e))}}})};var I5="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z";var W5="M12 2C6.5 2 2 6.5 2 12S6.5 22 12 22 22 17.5 22 12 17.5 2 12 2M12 20C7.59 20 4 16.41 4 12S7.59 4 12 4 20 7.59 20 12 16.41 20 12 20M16.59 7.58L10 14.17L7.41 11.59L6 13L10 17L18 9L16.59 7.58Z";var G5="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z";var U5="M19,4H15.5L14.5,3H9.5L8.5,4H5V6H19M6,19A2,2 0 0,0 8,21H16A2,2 0 0,0 18,19V7H6V19Z";var $5="M20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18,2.9 17.35,2.9 16.96,3.29L15.12,5.12L18.87,8.87M3,17.25V21H6.75L17.81,9.93L14.06,6.18L3,17.25Z";var S1=class extends S{constructor(){super();this.items=[]}render(){return d`
...
`;var z5=["days","weeks","months"];function j5(C){return{days:i("intervals.days",C),weeks:i("intervals.weeks",C),months:i("intervals.months",C)}}var L1=()=>({title:"",trigger_type:"time",interval_value:"",interval_type:"days",last_performed:"",icon:"",label:[],tag:"",count_entity_id:"",count_threshold:"",runtime_entity_id:"",runtime_threshold:"",area:"",description:"",group_id:"",notifications_enabled:!1,notification_target:"",notification_time:"09:00",notification_url:"",notify_when:"due_and_overdue",notify_days_before_due:""}),q5=(C,H,V)=>({title:C.title,trigger_type:C.trigger_type??"time",interval_value:C.interval_value,interval_type:C.interval_type,last_performed:C.last_performed??"",icon:C.icon??"",label:V.map(L=>L.label_id),tag:C.tag_id??"",count_entity_id:C.count_entity_id??"",count_threshold:C.count_threshold??"",runtime_entity_id:C.runtime_entity_id??"",runtime_threshold:C.runtime_threshold??"",area:H?.area_id??"",description:C.description??"",group_id:C.group_id??"",notifications_enabled:C.notifications_enabled??!1,notification_target:C.notification_target??"",notification_time:C.notification_time??"09:00",notification_url:C.notification_url??"",notify_when:C.notify_when??"due_and_overdue",notify_days_before_due:C.notify_days_before_due??""}),a0=C=>({name:"trigger_type",required:!0,selector:{select:{options:[{value:"time",label:i("trigger_types.time",C)},{value:"count",label:i("trigger_types.count",C)},{value:"runtime",label:i("trigger_types.runtime",C)}],mode:"dropdown"}}}),A0=(C,H)=>C.trigger_type==="count"?[{name:"count_entity_id",required:!0,selector:{entity:{}}},{name:"count_threshold",required:!0,selector:{number:{min:1,mode:"box"}}}]:C.trigger_type==="runtime"?[{name:"runtime_entity_id",required:!0,selector:{entity:{filter:{domain:"sensor"}}}},{name:"runtime_threshold",required:!0,selector:{number:{min:.1,step:.1,mode:"box"}}}]:[{name:"interval_value",required:!0,selector:{number:{min:1,mode:"box"}}},{name:"interval_type",required:!0,selector:{select:{options:z5.map(V=>({value:V,label:j5(H)[V]})),mode:"dropdown"}}}],g2=(C,H)=>({name:"group_id",selector:{select:{options:[{value:"",label:i("common.ungrouped",H)},...C.map(V=>({value:V,label:V}))],mode:"dropdown",custom_value:!0}}}),$1=(C,H)=>[{name:"title",required:!0,selector:{text:{}}},a0(H),...A0(C,H),{name:"last_performed",selector:{date:{}}}],Q1=(C,H)=>[g2(C,H),{name:"icon",selector:{icon:{}}},{name:"tag",selector:{entity:{filter:{domain:"tag"}}}},{name:"area",selector:{area:{}}},{name:"label",selector:{label:{multiple:!0}}}],z1=C=>({name:"description",selector:{text:C?{multiline:!0}:{}}}),j1=(C,H,V)=>{let L={name:"notifications_enabled",selector:{boolean:{}}};return C.notifications_enabled?[L,{name:"notification_target",selector:{select:{options:[{value:"",label:i("common.none",V)},...H.map(M=>({value:M,label:M}))],mode:"dropdown",custom_value:!0}}},{name:"notify_when",selector:{select:{options:[{value:"due",label:i("notifications.when.due",V)},{value:"overdue",label:i("notifications.when.overdue",V)},{value:"due_and_overdue",label:i("notifications.when.due_and_overdue",V)}],mode:"dropdown"}}},...C.trigger_type==="time"?[{name:"notify_days_before_due",selector:{number:{min:1,mode:"box"}}}]:[],{name:"notification_time",selector:{time:{no_second:!0}}},{name:"notification_url",selector:{text:{}}}]:[L]},q1=C=>C.title?.trim()?C.trigger_type==="count"?!!(C.count_entity_id?.trim()&&C.count_threshold):C.trigger_type==="runtime"?!!(C.runtime_entity_id?.trim()&&C.runtime_threshold):!!(C.interval_value&&C.interval_type):!1,K1=C=>{if(!C){let o=new Date;return o.setHours(0,0,0,0),o.toISOString()}let[H,V,L]=C.split("T")[0].split("-"),M=Number(H),e=Number(V),r=Number(L);if(isNaN(M)||isNaN(e)||isNaN(r))return null;let t=new Date(M,e-1,r);return t.setHours(0,0,0,0),t.toISOString()},K5=C=>({notifications_enabled:C.notifications_enabled??!1,notification_target:C.notification_target?.trim()||null,notification_time:C.notification_time?.trim()||"09:00",notification_url:C.notification_url?.trim()||null,notify_when:C.notify_when||"due_and_overdue",notify_days_before_due:C.notify_days_before_due===""||C.notify_days_before_due==null?null:Number(C.notify_days_before_due)}),X5=C=>{let H=C.trigger_type==="count",V=C.trigger_type==="runtime";return{trigger_type:C.trigger_type||"time",interval_value:H||V?1:Number(C.interval_value),interval_type:H||V?"days":C.interval_type,count_entity_id:H&&C.count_entity_id?.trim()||null,count_threshold:H?Number(C.count_threshold):0,runtime_entity_id:V&&C.runtime_entity_id?.trim()||null,runtime_threshold:V?Number(C.runtime_threshold):0}},Y5=(C,H)=>{let V=X5(C);return{title:C.title.trim(),interval_value:V.interval_value,interval_type:V.interval_type,trigger_type:V.trigger_type,last_performed:H,tag_id:C.tag?.trim()||void 0,icon:C.icon?.trim()||"mdi:calendar-check",labels:C.label??[],area_id:C.area?.trim()||void 0,description:C.description||void 0,group_id:C.group_id?.trim()||void 0,...V.count_entity_id?{count_entity_id:V.count_entity_id,count_threshold:V.count_threshold}:{},...V.runtime_entity_id?{runtime_entity_id:V.runtime_entity_id,runtime_threshold:V.runtime_threshold}:{},...K5(C)}},J5=(C,H)=>({title:C.title.trim(),...X5(C),last_performed:H,icon:C.icon?.trim()||"mdi:calendar-check",labels:C.label,tag_id:C.tag?.trim()||null,area_id:C.area?.trim()||null,description:C.description??"",group_id:C.group_id?.trim()||null,...K5(C)});var X=class extends S{constructor(){super(...arguments);this.groups=[];this._formData=L1();this._advancedOpen=!1;this._handleFieldChanged=(V,L)=>{L.stopPropagation(),this._formData={...this._formData,[V]:L.detail.value}};this._renderField=V=>G1({hass:this.hass,keyPrefix:"panel.cards.new.fields",data:this._formData,onChange:this._handleFieldChanged},V)}async _handleAddTaskClick(){if(!q1(this._formData)){f(this,i("panel.cards.new.alerts.required",this.hass.language));return}let V=K1(this._formData.last_performed);if(V===null){f(this,i("common.invalid_date",this.hass.language));return}try{let L=this._formData.title.trim();await k5(this.hass,Y5(this._formData,V)),this._formData=L1(),this.dispatchEvent(new CustomEvent("task-added",{detail:{title:L},bubbles:!0,composed:!0}))}catch(L){console.error("Failed to add task:",L),f(this,i("panel.cards.new.alerts.error",this.hass.language))}}render(){return this.hass?d`
            <div class="basic-row">
            ...
            >
                ...
                <div class="fields-grid">
                    ${j1(this._formData,I1(this.hass),this.hass.language).map(this._renderField)}
                </div>
            </ha-expansion-panel>
        `:d``}};X.styles=[b,U1,k`
    ...
    `],m([s()],X.prototype,"hass",2),m([s({attribute:!1})],X.prototype,"groups",2),m([h()],X.prototype,"_formData",2);customElements.get("hm-task-form")||customElements.define("hm-task-form",X);var P=class extends S{constructor(){super(...arguments);this.registry=[];this.labelRegistry=[];this.groups=[];this._taskId=null;this._formData=L1();this._handleFieldChanged=(V,L)=>{L.stopPropagation(),this._formData={...this._formData,[V]:L.detail.value}};this._renderField=V=>G1({hass:this.hass,keyPrefix:"panel.dialog.edit_task.fields",data:this._formData,onChange:this._handleFieldChanged},V)}async open(V){try{let L=await b5(this.hass,V),M=this.registry.find(r=>r.unique_id===L.id),e=M?this.labelRegistry.filter(r=>M.labels.includes(r.label_id)):[];this._formData=q5(L,M,e),this._taskId=L.id}catch(L){console.error("Failed to fetch task for edit:",L)}}async _handleSaveClick(){if(!this._taskId)return;if(!q1(this._formData)){f(this,i("panel.cards.new.alerts.required",this.hass.language));return}let V=K1(this._formData.last_performed);if(V===null){f(this,i("common.invalid_date",this.hass.language));return}try{await W1(this.hass,{task_id:this._taskId,updates:J5(this._formData,V)}),this._close()}catch(L){console.error("Failed to update task:",L),f(this,i("panel.dialog.edit_task.alerts.error",this.hass.language))}}_close(){this._taskId=null,this._formData=L1()}async _handleTestNotification(){let V=this.registry.find(L=>L.unique_id===this._taskId);if(V)try{await this.hass.callService("home_maintenance","send_task_notification",{entity_id:V.entity_id})}catch(L){console.error("Failed to send test notification:",L),f(this,i("panel.dialog.edit_task.alerts.test_error",this.hass.language))}}render(){if(!this.hass||!this._taskId)return d``;let V=this.hass.language;return d`
            <ha-dialog
                open
            ...
            >
                ...
                <div class="fields-grid">
                    ${j1(this._formData,I1(this.hass),V).map(this._renderField)}
                </div>
    ...
    `],m([s()],P.prototype,"hass",2),m([s({attribute:!1})],P.prototype,"registry",2),m([s({attribute:!1})],P.prototype,"labelRegistry",2),m([s({attribute:!1})],P.prototype,"groups",2),m([h()],P.prototype,"_taskId",2),m([h()],P.prototype,"_formData",2);customElements.get("hm-edit-dialog")||customElements.define("hm-edit-dialog",P);var h1=class extends S{constructor(){super(...arguments);this._opts=null}open(V){this._opts=V}_close(){this._opts=null}_handleConfirm(){let V=this._opts?.onConfirm;this._close(),V?.()}_renderButtons(){return d`
        ...
        `:d``}};h1.styles=b,m([h()],h1.prototype,"_opts",2);customElements.get("hm-confirm-dialog")||customElements.define("hm-confirm-dialog",h1);var _=class extends S{constructor(){super(...arguments);this.groups=[];this._newGroupName="";this._renamingGroup=null;this._renameValue=""}async _handleCreate(){let V=this._newGroupName.trim();if(V){if(this.groups.includes(V)){f(this,i("panel.cards.groups.alerts.exists",this.hass.language,"{title}",V));return}try{await P5(this.hass,V),this._newGroupName=""}catch(L){console.error("Failed to create group:",L),f(this,i("panel.cards.groups.alerts.error",this.hass.language))}}}_startRename(V){this._renamingGroup=V,this._renameValue=V}async _handleRename(){let V=this._renamingGroup,L=this._renameValue.trim();if(!V||!L||V===L){this._renamingGroup=null;return}if(this.groups.includes(L)){f(this,i("panel.cards.groups.alerts.exists",this.hass.language,"{title}",L));return}this._renamingGroup=null;try{await _5(this.hass,V,L)}catch(M){console.error("Failed to rename group:",M),f(this,i("panel.cards.groups.alerts.rename_error",this.hass.language))}}_handleDelete(V){let L=this.hass.language;this._confirmDialog?.open({heading:i("panel.cards.groups.confirm_delete_title",L),message:i("panel.cards.groups.confirm_delete",L,"{title}",V),confirmLabel:i("panel.cards.groups.actions.delete",L),cancelLabel:i("common.cancel",L),destructive:!0,onConfirm:()=>this._deleteGroup(V)})}async _deleteGroup(V){try{await F5(this.hass,V)}catch(L){console.error("Failed to delete group:",L),f(this,i("panel.cards.groups.alerts.delete_error",this.hass.language))}}render(){if(!this.hass)return d``;let V=this.hass.language;return d`
        ...
        `}};D.styles=b,m([s()],D.prototype,"hass",2),m([s({attribute:!1})],D.prototype,"groups",2),m([h()],D.prototype,"_task",2),m([h()],D.prototype,"_groupId",2);customElements.get("hm-move-dialog")||customElements.define("hm-move-dialog",D);var n0=300,y=class extends S{constructor(){super(...arguments);this.tags=null;this.tasks=[];this.groups=[];this.config=null;this.registry=[];this.labelRegistry=[];this._reload=new D1(()=>this._loadData(),n0)}connectedCallback(){super.connectedCallback(),this._initialize()}disconnectedCallback(){super.disconnectedCallback(),this._reload.cancel(),this._unsubscribe?.(),this._unsubscribe=void 0}async _initialize(){await u5();let[V,L]=await Promise.all([f5(this.hass),E5(this.hass)]);this.tags=V,this.config=L,await this._loadData();try{this._unsubscribe=await R5(this.hass,()=>this._reload.schedule())}catch(M){console.error("Failed to subscribe to task updates:",M)}}async _loadData(){let[V,L,M,e]=await Promise.all([y5(this.hass),T5(this.hass),g5(this.hass),O5(this.hass)]);this.tasks=V,this.groups=L,this.registry=M,this.labelRegistry=e}_handleComplete(V){let L=this.tasks.find(M=>M.id===V.detail.taskId);L&&D5(this,this._confirmDialog,this.hass,L)}_handleMenuAction(V){let{taskId:L,action:M}=V.detail;if(M==="edit")this._editDialog?.open(L);else if(M==="move"){let e=this.tasks.find(r=>r.id===L);e&&this._moveDialog?.open(e)}else M==="delete"&&this._handleRemove(L)}_handleRemove(V){let L=this.tasks.find(M=>M.id===V);N5(this,this._confirmDialog,this.hass,L,V)}_handleTaskAdded(V){f(this,i("card.add_task.added",this.hass.language,"{title}",V.detail?.title??""))}render(){return this.hass?!this.tasks||!this.tags?d`<p>${i("common.loading",this.hass.language)}</p>`:d`
            <div class="header">
                <div class="toolbar">
